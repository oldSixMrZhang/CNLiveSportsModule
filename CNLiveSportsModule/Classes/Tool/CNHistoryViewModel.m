...
//    NSLog(@"%@",dict);
    ...
    for (NSDictionary *point in dict[@"points"]) {
        CNHistoryTrackPoint *p = [[CNHistoryTrackPoint alloc] init];
    ...
    if ([dict[@"tag"] unsignedIntValue] == 1) {
        ...
        dispatch_group_notify(self.historyDispatchGroup, GLOBAL_QUEUE, ^{
            ...
            [self.points sortWithOptions:NSSortStable usingComparator:^NSComparisonResult(CNHistoryTrackPoint *  _Nonnull obj1, CNHistoryTrackPoint *  _Nonnull obj2) {
            ...
            if (FALSE == self.param.isProcessed) {
                ...
                for (size_t i = 0; i < self.points.count - 1; i++) {
                    CNHistoryTrackPoint *point1 = (CNHistoryTrackPoint *)self.points[i];
                    CNHistoryTrackPoint *point2 = (CNHistoryTrackPoint *)self.points[i + 1];
                    ...
                    ((CNHistoryTrackPoint *)self.points[i]).direction = [self mapGeometryDirectionToGPSDirection:degreeDirection];
                }
            }
            if (self.completionHandler) {
