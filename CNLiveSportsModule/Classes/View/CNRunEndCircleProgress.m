...
@interface CNRunEndCircleProgress ()<CAAnimationDelegate>
...
@implementation CNRunEndCircleProgress
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _startAngle = CNCircleDegreeToRadian(startAngle);
...
//初始化数据
- (void)initialization {
    self.backgroundColor = [UIColor greenColor];//[UIColor colorWithRed:250/255.0 green:112/255.0 blue:112/255.0 alpha:1.0];
    _pathBackColor = [UIColor lightGrayColor];
    _pathFillColor = [UIColor redColor];
    
    _strokeWidth = 10;//线宽默认为10
    _startAngle = CNCircleDegreeToRadian(0);//圆起点位置
    _reduceAngle = CNCircleDegreeToRadian(0);//整个圆缺少的角度
...
- (UIImageView *)pointImage {
    if (!_pointImage) {
        ...
        NSBundle *resourcesBundle = [NSBundle bundleWithPath:[mainBundle pathForResource:@"CNCircleProgress" ofType:@"bundle"]];
        _pointImage.image = [UIImage imageNamed:@"circle_point1" inBundle:resourcesBundle compatibleWithTraitCollection:nil];
    }
    return _pointImage;
}

- (CNRunEndCountingLabel *)progressLabel {
    if (!_progressLabel) {
        _progressLabel = [[CNRunEndCountingLabel alloc] init];
...
- (void)setStartAngle:(CGFloat)startAngle {
    if (_startAngle != CNCircleDegreeToRadian(startAngle)) {
        _startAngle = CNCircleDegreeToRadian(startAngle);
        [self setNeedsLayout];
    }
}
...
- (void)setReduceAngle:(CGFloat)reduceAngle {
    if (_reduceAngle != CNCircleDegreeToRadian(reduceAngle)) {
        if (reduceAngle>=360) {
            return;
        }
        _reduceAngle = CNCircleDegreeToRadian(reduceAngle);
        [self setNeedsLayout];
    }
}
...
- (void)layoutSubviews {
    ...
    self.realWidth = MIN(CNCircleSelfWidth, CNCircleSelfHeight);
